...
public class PedidoItemController {
	...
	public ResponseEntity<Object> inserir(@Valid @RequestBody PedidoItemInserirDTO itemPed) {
		return ResponseEntity.ok(pedidoItemService.inserir(itemPed));
	}
	...
	@ApiResponses(value = { @ApiResponse(code = 201, message = "Atualiza um item de pedido"),
			...
			@ApiResponse(code = 404, message = "Recurso não encontrado"),
			@ApiResponse(code = 500, message = "Erro de servidor") })
	public ResponseEntity<Object> atualizarPorId(@PathVariable(name = "id") Long id, @Valid @RequestBody PedidoItemInserirDTO itemPed) {
